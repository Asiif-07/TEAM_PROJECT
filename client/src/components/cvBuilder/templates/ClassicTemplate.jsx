...
const ContactChunk = ({ icon: LucideIcon, text, link, color }) => {
  if (!text) return null;
  const content = (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      {React.createElement(LucideIcon, { size: 12, color: color, strokeWidth: 2.5 })}
...
// Classic section header with a bold color left-border rule
const ClassicSection = ({ title, children, color }) => {
...
export default function ClassicTemplate({ data, accentColor = "#DC2626" }) {
  ...
  return (
    <Box className="cv-document" sx={{ p: 6, bgcolor: "#fff", minHeight: "297mm", fontFamily: "'Times New Roman', serif", color: "#111827", '@media print': { boxShadow: 0 } }}>
      ...
      <Box sx={{ textAlign: "center", mb: 1 }}>
        ...
        <Box sx={{ width: 60, height: 3, bgcolor: accentColor, mx: "auto", borderRadius: 2, mb: 1 }} />
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 2.5, my: 2 }}>
        <ContactChunk icon={Mail} text={personalInfo.email} link={`mailto:${personalInfo.email}`} color={accentColor} />
        <ContactChunk icon={Phone} text={personalInfo.phone} color={accentColor} />
        <ContactChunk icon={MapPin} text={personalInfo.address} color={accentColor} />
        {personalInfo.linkedin && <ContactChunk icon={Linkedin} text={personalInfo.linkedin} link={personalInfo.linkedin} color={accentColor} />}
        {personalInfo.github && <ContactChunk icon={Github} text={personalInfo.github} link={personalInfo.github} color={accentColor} />}
      </Box>

      <Divider sx={{ borderColor: accentColor, opacity: 0.6, mb: 4 }} />

      {/* SUMMARY */}
      {personalInfo.about && (
        <ClassicSection title="Professional Profile" color={accentColor}>
      ...
      {/* EXPERIENCE */}
      <ClassicSection title="Work Experience" color={accentColor}>
        {experience?.map((exp, index) => (
          <ExperienceItem key={index} role={exp.role} company={exp.company} duration={exp.duration} description={exp.description} color={accentColor} />
        ))}
      </ClassicSection>
      ...
      <ClassicSection title="Education" color={accentColor}>
        {education?.map((edu, index) => (
          <EducationItem key={index} degree={edu.degree} institute={edu.institute} year={edu.year} color={accentColor} />
        ))}
      </ClassicSection>

      <Grid container spacing={4}>
        <Grid item xs={6}>
          <ClassicSection title="Skills" color={accentColor}>
            <SkillList skills={skills} color={accentColor} />
          </ClassicSection>
        </Grid>
        <Grid item xs={6}>
          <ClassicSection title="Projects" color={accentColor}>
