...
const BlogDetailsHero = ({ blog }) => {
  ...
  const title = blog?.title || "How to Optimize Your LinkedIn Profile for Recruiters";
  const subtitle = blog?.description?.replace(/<[^>]*>?/gm, '').substring(0, 160) + "..." || "A step-by-step guide to creating a LinkedIn profile that attracts recruiters and opens doors to new opportunities.";
  const badgeText = blog?.category?.name || "AI-Enhance Career Services";
